...
resort = Table('resort', pre_meta,
    ...
    Column('la_n', VARCHAR(length=10)),
    Column('lo_n', VARCHAR(length=10)),
)
...
def upgrade(migrate_engine):
    ...
    pre_meta.tables['resort'].columns['la_n'].drop()
    pre_meta.tables['resort'].columns['lo_n'].drop()
    post_meta.tables['resort'].columns['la'].create()
    post_meta.tables['resort'].columns['lo'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    pre_meta.tables['resort'].columns['la_n'].create()
    pre_meta.tables['resort'].columns['lo_n'].create()
    post_meta.tables['resort'].columns['la'].drop()
    post_meta.tables['resort'].columns['lo'].drop()
