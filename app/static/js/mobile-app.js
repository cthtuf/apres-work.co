            ...
            try {
                $('#form_contact').on('submit', function(e){
                    ...
                    $.ajax({
                        ...
                        switch(rpo.CODE){
                            case "0":
                                $('.contact-form').append($('<div>', { 'class' : 'alert alert-success', text : rpo.TEXT}));
                                $('#form_contact').addClass('hidden');
                                break;
                            default:
                                $('.contact-form').append($('<div>', { 'class' : 'alert alert-warning', text : rpo.TEXT}));
                                break;
                        }
                    }).fail(function(){
                        $('.contact-form').append($('<div>', { 'class' : 'alert alert-warning', text : 'Произошла ошибка. Попробуйте еще раз'}));
                    }).always(function(){
